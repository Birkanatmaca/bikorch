...
describe('Cursor terminal isolation', () => {
  it('opens both accounts and logs out only the selected account', async () => {
    await ptyManager.create({ sessionId: 'session-a', projectId: 'project-1234', kind: 'cursor', accountId: 'a', cwd: '.' }, contents)
    await ptyManager.create({ sessionId: 'session-b', projectId: 'project-1234', kind: 'cursor', accountId: 'b', cwd: '.' }, contents)
  ...
  it('relogin closes only the same account terminal and leaves the other account running', async () => {
    await ptyManager.create({ sessionId: 'a', projectId: 'project-1234', kind: 'cursor', accountId: 'a', cwd: '.' }, contents)
    await ptyManager.create({ sessionId: 'b', projectId: 'project-1234', kind: 'cursor', accountId: 'b', cwd: '.' }, contents)
    const [a, b] = mocks.spawn.mock.results.map((result) => result.value)
    await ptyManager.create({ sessionId: 'a-login', projectId: 'project-1234', kind: 'cursor', accountId: 'a', launchMode: 'login', cwd: '.' }, contents)
    expect(a.kill).toHaveBeenCalledTimes(1)
    expect(b.kill).not.toHaveBeenCalled()
