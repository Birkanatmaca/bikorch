import type { CliUsageBreakdown, CliUsageInfo } from '@shared/contracts/usage'
import {
...
export function parseCursorDashboardUsage(usage: JsonRecord, planResponse: JsonRecord, hardLimit: JsonRecord): Pick<CliUsageInfo, 'status' | 'detail' | 'planType' | 'primary' | 'breakdown'> {
  ...
  const used = number(plan.totalPercentUsed) ??
  ...
  const breakdown: CliUsageBreakdown[] = [{ label: 'Included', value: `${used.toFixed(1)}% used`, usedPercent: clamp(used) }]
  for (const [label, field] of [['Auto', 'autoPercentUsed'], ['API', 'apiPercentUsed']]) {
    const value = number(plan[field])
    if (value !== undefined) breakdown.push({ label, value: `${value.toFixed(1)}% used`, usedPercent: clamp(value) })
  }
  ...
  return {
    status: 'available', detail: 'Bu hesabın doğrulanmış canlı Cursor kullanımı.', planType, breakdown,
    primary: { label: 'Included usage', usedPercent: clamp(used),
