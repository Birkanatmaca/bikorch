import { Gauge, PanelLeftClose, UsersRound } from 'lucide-react'
...
import { LimitsPanel } from '@renderer/components/usage/LimitsPanel'
...
export function LeftSidebar({ view, onHide }: LeftSidebarProps): React.JSX.Element {
  ...
  const isLimits = view === 'limits'
  const isAccounts = view === 'accounts'
  const title = isChanges ? 'Changes' : isLimits ? 'Limits' : isAccounts ? 'AI Accounts' : 'Files'
  ...
  return (
    <div className="panel-shell relative flex h-full flex-col overflow-hidden rounded-md border border-border bg-panel-bg shadow-sm">
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden">
        <div className={cn('h-full', (isChanges || isLimits || isAccounts) && 'hidden')}>
          <FileExplorerPanel />
        </div>
